 ...
 * @type {ErrorMessenger}
 */
export function ConsoleErrorMessenger(
  message,
  error = undefined,
  options = {},
  ...args
  ...
  return new Promise((resolve, reject) => {
    if (error) {
      global.console.error(
        formatMessage(message, args, options).then(() => {
          if (options.showTrace) {
...
/**
 * Check label of a timer, counter, or group.
 * @param {*} label The tested label.
 * @returns {string}The label.
 * @throws {SyntaxError} The label was invalid.
 */
export function checkLabel(label="default") {
  try {
    return `${label}`;
  } catch(err) {
    throw new SyntaxError("Invalid label", {cause: err});
  }
}
...
export function TimerSupport(logger) {
  ...
  function defaultTimeEnd(label = "default") {
    ...
    if (timers.has(label)) {
      return this.log(
        `${label}: ${currentTime - timers.get(label)}ms - timer ended`
      );
    } else {
   ...
   * @throws {SyntaxError} The label is invalid.
   */
  function timeLog(label = "default", ...messages) {
    const currentTime = Date.now();
    const actualLabel = checkLabel(label);
    if (timers.has(actualLabel)) {
      /** @todo Add support for messages. */
      return this.log(`${actualLabel}: ${currentTime - timers.get(actualLabel)}ms`);
    } else {
      return this.warn(`Timer "${actualLabel}" does not exist`);
    }
  }
...
/**
 * The log level reprsnts a log level entry.
 * @typedef {Object} LogLevel
 * @property {Readonly<number>} code The log level code.
 * @property {Readonly<string>} name THe name of the level.
 * @property {() => number} valueOf Converting the object to a primitive valuee of the code.
 * @property {() => string} toString Converting the object to a string of the log level name.
 */
...
export function formatMessage(message, messageArgs = [], options = {}) {
  ...
  return `${indent}${options.prefix ?? ""}${format(
    message,
    ...messageArgs
...
export class ConsoleLogger {
   ...
   * @param {Console} console The console used for output.
   */
  ...
  /**
   * The log levels of the console logger.
   * @type {LogLevel[]}
   */
  static LogLevels = ["all", "debug", "warn", "info", "error", "none"].map(
    (name, code) => {
      // Create an immutable log level entry.
      const result = {
        name,
        code,
        valueOf() {
   ...
   * @default
   */
  static INFO = ConsoleLogger.LogLevels.find((level) => level.name === "info");
  ...
  /**
   * Does the logger log a log level.
   * @param {string|number|LogLevel} logLevel The tested log level. 
   * @returns {boolean} True, if and only if the given log level is logged.
   */
  isLogged(logLevel) {
    switch (typeof logLevel) {
      case "string":
        return (
          ConsoleLogger.LogLevels.find((entry) => entry.name === logLevel)
            ?.code >= this.#logLevel.code
        );
      case "number":
        return logLevel >= this.#logLevel.code;
      case "object":
        if ("code" in logLevel) {
          return logLevel.code >= this.#logLevel.code;
        } else if ("valueOf" in logLevel) {
          return logLevel.valueOf() >= this.#logLevel.code;
        } else if ("toString" in logLevel) {
          const name = logLevel.toString();
          return (
            ConsoleLogger.LogLevels.find((entry) => entry.name === name)
              ?.code >= this.#logLevel.code
          );
        }
      default:
  ...
  set logLevel(level) {
    function checkLevel(level) {
      if (level === undefined) {
        throw new TypeError("Invalid logging level");
      }
      return level;
    }
    switch (typeof level) {
      case "string":
        this.#logLevel = checkLevel(
          ConsoleLogger.LogLevels.find((entry) => entry.name === level)
        );
        return;
      case "number":
        this.#logLevel = checkLevel(ConsoleLogger.LogLevels[level]);
        return;
      case "object":
        if (["code", "name"].every((prop) => prop in level)) {
          this.#logLevel = checkLevel(ConsoleLogger.LogLevel[level.code]);
          return;
        }
      default:
        this.#logLevel = checkLevel(undefined);
    }
  }

  /**
   * The current log level.
   * @type {string} Current log.
   */
  get logLevel() {
  ...
  log(message, options = {}) {
    return Promise.resolve(
      this.#console.log(this.formatMessage(message, options))
    );
  }
  ...
  warn(message, options = {}) {
    ...
    if (this.isLogged(logLevel)) {
      return this.log(message, {
        ...options,
        prefix: `[${logLevel}]${options.prefix ?? ""}`,
      });
    } else {
      return Promise.resolve();
    }
...
export class Logger {
  ...
  count(label="default") {
    ...
    return this.log(`${label}: ${this.#counters.get(label)}`);
    } else {
  ...
  timeEnd(label = "debug") {
    ...
    return new Promise((resolve, reject) => {
      if (this.#timers.delete(label)) {
        resolve(
          this.log(
            `${label}: ${currentDate - (startDate ?? 0)}ms - Timer ended`
          )
        );
      } else {
  ...
  timeLog(label = "debug", ...args) {
    ...
    return new Promise((resolve, reject) => {
      if (startDate !== undefined) {
        resolve(
          this.log(
            `${label}: ${currentDate - (startDate ?? 0)}ms ${formatMessage(
              "",
              args
            )}`
          )
