 ...
 * the undefined or null value given.
 */
export function andThenDo(value, definedFn, undefinedFn = undefined, nullFn = undefined) {
    if (value === undefined) {
 ...
 */
export function orElse(value, undefinedDefaultValue, nullDefaultValue = undefined) {
    if (value === undefined || (value === null && nullDefaultValue === undefined)) {
...
export class RestResource extends BasicDao {
    ...
    static checkalueFormatter(valueFormatFn, linkGenerator = undefined) {
        if (valueFormatFn === undefined) {
             ...
             */
            const resultFn = ( /** @type {TYPE} */ value, /** @type {ID|undefined} */ id = undefined) => {
                try {
                    /**
                     * @type {RestData}
                     */
                    const result = {
                        ...
                        links: (linkGenerator === undefined ? undefined : linkGenerator(value, id))
                    };
                    return result;
                } catch (err) {
        ...
        } else if (valueFormatFn instanceof Function && valueFormatFn.length >= 1 && valueFormatFn.length <= 2) {
            /**
             * The result value formatting function.
             * @type {ValueFormatter<TYPE,ID>}
             */
    ...
    static checkValueParser(valueParserFn) {
        ...
        } else if (valueParserFn instanceof Function && valueParserFn.lnegth === 1) {
            return (/** @type {RestData} */ restValue) => {
                try {
    ...
    constructor(dao, options = {}) {
        ...
        this.#parseData = andThen(options.formatValue, undefined, options.parseData);
        this.#formatData = options.formatData;
        this.#formatValue = orElse(options.formatValue, this.checkValueFormatter(options.formatValue, options.linkGenerator));
    ...
    checkIdFormatter(formatterFn) {
        return ( /** @type {RestResource<ID,TYPE>}*/ RestResource).checkIdFormatter(formatterFn);
    }
    ...
    checkValueFormatter(valueStringifier, linkGenerator = undefined) {
        return ( /** @type {RestResource<ID,TYPE>}*/ RestResource).checkValueFormatter(valueStringifier, linkGenerator);
    }
    ...
    checkIdParser(parserFn) {
        return ( /** @type {RestResource<ID,TYPE>}*/ RestResource).checkIdParser(parserFn);
    }
    ...
    checkValueParser(parserFn) {
        return ( /** @type {RestResource<ID,TYPE>}*/ RestResource).checkValueParser(parserFn);
    }
     ...
     */
    all() {
        return this.#dao.all().then((entries) => (entries.map(([id, value]) => ([this.formatId(id), this.formatValue(value, id)]))));
    }
    ...
    one(id) {
         ...
         */
        return this.#dao.one(daoId).then(( /** @type {TYPE} */ result) => (this.formatValue(result, daoId)));
    }
    ...
    update(id, value) {
        return new Promise((resolve, reject) => {
            this.parseId(id).then((resourceId) => (
                this.parseValue(value).then((resourceValue) => (
                    this.#dao.update(resourceId, resourceValue))))).then(resolve, reject);
        })
    }
    ...
    remove(id) {
        return new Promise((resolve, reject) => {
            this.parseId(id).then((resourceId) => (
                this.#dao.remove(resourceId))).then(resolve, reject);
        })
    }
}
...
export class RestServer {
    ...
    hasResource(path, resourceName) {
        return this.#resources.find(([entryPath, entryName]) => (path === entryPath && resourceName == entryName)) !== undefined;
    }
    ...
    unregeisterResource(basePath, resourceName) {
        this.#resources = this.#resources.filter(([entryPath, entryName]) => (entryPath !== path || entryName !== entryName))
    }
